...
# -*- coding: utf-8 -*-
"""
This experiment was created using PsychoPy2 Experiment Builder (v1.81.00), Tue Feb 10 13:55:40 2015
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath=u'/Users/dbl-a/psychopy-experiments/MRI-nBack/MRI-nBack.psyexp',
...
image = visual.ImageStim(win=win, name='image',
    image=u'N-back Instructions.jpg', mask=None,
...
# store data for nBackLoop (TrialHandler)
nBackLoop.addData('key_resp_18.keys',key_resp_18.keys)
if key_resp_18.keys != None:  # we had a response
    nBackLoop.addData('key_resp_18.rt', key_resp_18.rt)
...
# store data for nBackLoop (TrialHandler)
nBackLoop.addData('key_resp_17.keys',key_resp_17.keys)
if key_resp_17.keys != None:  # we had a response
    nBackLoop.addData('key_resp_17.rt', key_resp_17.rt)
...
# store data for nBackLoop (TrialHandler)
nBackLoop.addData('key_resp_2.keys',key_resp_2.keys)
if key_resp_2.keys != None:  # we had a response
    nBackLoop.addData('key_resp_2.rt', key_resp_2.rt)
...
# store data for nBackLoop (TrialHandler)
nBackLoop.addData('key_resp_7.keys',key_resp_7.keys)
if key_resp_7.keys != None:  # we had a response
    nBackLoop.addData('key_resp_7.rt', key_resp_7.rt)
...
# store data for nBackLoop (TrialHandler)
nBackLoop.addData('key_resp_4.keys',key_resp_4.keys)
if key_resp_4.keys != None:  # we had a response
    nBackLoop.addData('key_resp_4.rt', key_resp_4.rt)
...
while continueRoutine:
    ...
    if key_resp_9.status == STARTED and frameN >= 18:
    ...
    if image.status == STARTED and frameN >= 18:
...
# store data for nBackLoop (TrialHandler)
nBackLoop.addData('key_resp_9.keys',key_resp_9.keys)
if key_resp_9.keys != None:  # we had a response
    nBackLoop.addData('key_resp_9.rt', key_resp_9.rt)


# set up handler to look after randomisation of conditions etc
nBackLoop = data.TrialHandler(nReps=1, method='sequential', 
    extraInfo=expInfo, originPath=u'/Users/dbl-a/psychopy-experiments/MRI-nBack/MRI-nBack.psyexp',
    trialList=data.importConditions(u'stimOrder.csv'),
    seed=None, name='nBackLoop')
thisExp.addLoop(nBackLoop)  # add the loop to the experiment
thisNBackLoop = nBackLoop.trialList[0]  # so we can initialise stimuli with some values
# abbreviate parameter names if possible (e.g. rgb=thisNBackLoop.rgb)
if thisNBackLoop != None:
    for paramName in thisNBackLoop.keys():
        exec(paramName + '= thisNBackLoop.' + paramName)

for thisNBackLoop in nBackLoop:
    currentLoop = nBackLoop
    # abbreviate parameter names if possible (e.g. rgb = thisNBackLoop.rgb)
    if thisNBackLoop != None:
        for paramName in thisNBackLoop.keys():
            exec(paramName + '= thisNBackLoop.' + paramName)
    ...
    while continueRoutine:
        ...
        if text_7.status == STARTED and frameN >= 120:
        ...
        if frameN >= 59 and key_resp_11.status == NOT_STARTED:
            ...
            event.clearEvents(eventType='keyboard')
        if key_resp_11.status == STARTED and frameN >= 120:
    ...
    # store data for nBackLoop (TrialHandler)
    nBackLoop.addData('key_resp_11.keys',key_resp_11.keys)
    if key_resp_11.keys != None:  # we had a response
        nBackLoop.addData('key_resp_11.rt', key_resp_11.rt)
    ...
    ns.send_event( 'lttr', label="Letter Presented", timestamp=egi.ms_localtime(), table = {'ltr-' : letters, 'time' : egi.ms_localtime()} ) 
    ...
    while continueRoutine:
        ...
        if text_8.status == STARTED and frameN >= 60:
        ...
        if frameN >= 0 and key_resp_10.status == NOT_STARTED:
            ...
            event.clearEvents(eventType='keyboard')
        if key_resp_10.status == STARTED and frameN >= 60:
        ...
        if key_resp_10.status == STARTED:
            ...
            if len(theseKeys) > 0:  # at least one key was pressed
                ...
                # was this 'correct'?
                if (key_resp_10.keys == str(correctButton)) or (key_resp_10.keys == correctButton):
                    key_resp_10.corr = 1
                else:
                    key_resp_10.corr = 0
        ...
        if key_resp_12.status == STARTED and frameN >= 60:
    ...
    if key_resp_10.keys in ['', [], None]:  # No response was made
       ...
       if str(correctButton).lower() == 'none': key_resp_10.corr = 1  # correct non-response
       else: key_resp_10.corr = 0  # failed to respond (incorrectly)
    # store data for nBackLoop (TrialHandler)
    nBackLoop.addData('key_resp_10.keys',key_resp_10.keys)
    nBackLoop.addData('key_resp_10.corr', key_resp_10.corr)
    if key_resp_10.keys != None:  # we had a response
        nBackLoop.addData('key_resp_10.rt', key_resp_10.rt)
    '''
    ...
    position = position + 1
    '''
    ns.send_event( 'rsp-', label="Subject Response", timestamp=egi.ms_localtime(), table = {'resp': key_resp_10.keys, 'eval': '?', 'time' : egi.ms_localtime()} ) 
    # check responses
    if key_resp_12.keys in ['', [], None]:  # No response was made
       key_resp_12.keys=None
    # store data for nBackLoop (TrialHandler)
    nBackLoop.addData('key_resp_12.keys',key_resp_12.keys)
    if key_resp_12.keys != None:  # we had a response
        nBackLoop.addData('key_resp_12.rt', key_resp_12.rt)
    thisExp.nextEntry()
    
# completed 1 repeats of 'nBackLoop'

# get names of stimulus parameters
if nBackLoop.trialList in ([], [None], None):  params = []
else:  params = nBackLoop.trialList[0].keys()
# save data for this loop
nBackLoop.saveAsExcel(filename + '.xlsx', sheetName='nBackLoop',
    stimOut=params,
    dataOut=['n','all_mean','all_std', 'all_raw'])
nBackLoop.saveAsText(filename + 'nBackLoop.csv', delim=',',
    stimOut=params,
    dataOut=['n','all_mean','all_std', 'all_raw'])
...
# store data for nBackLoop (TrialHandler)
nBackLoop.addData('key_resp_14.keys',key_resp_14.keys)
if key_resp_14.keys != None:  # we had a response
    nBackLoop.addData('key_resp_14.rt', key_resp_14.rt)
